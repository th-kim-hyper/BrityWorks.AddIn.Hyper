...
var BrityWorksExtension = BrityWorksExtension || (() => {
    class Inspector {
        constructor() {
			...
			this.browserAppData = this.browser || this.chrome;
            ...
            this.applyStorageOptions = this.applyStorageOptions.bind(this);
            this.getStorageDataFromWorker = this.getStorageDataFromWorker.bind(this);
        ...
        preventClick(e) {
            ...
            if (this.isMultiSelectMode()) {
				if (this.multipleSelectModeMousePositionNode == null) {
					this.clickedNode = e.target;  // 타이밍 이슈 가능하면 입력하지 않도록
				}
        ...
        async getXPathData(eventTarget) {
            if (eventTarget.id !== this.contentNode) {
                ...
                if (this.isShowQueryGuide == false)
                    return;
        ...
        async getStorageDataFromWorker(searchkey) {
            let data = await new Promise((resolve) => {
                chrome.runtime.sendMessage({ title: 'getStorageOption', key: searchkey }, function (response) {
        ...
        async applyStorageOptions(key) {
            switch (key) {
                case 'isShowQueryGuide':
                    this.isShowQueryGuide = await this.getStorageDataFromWorker('isShowQueryGuide');
                    break;
                default:
                    this.isShowQueryGuide = await this.getStorageDataFromWorker('isShowQueryGuide');
                    break;
