...
namespace HyperInfo.Lib.Net461.Mail
{
    public static class HyperMail
    {
        ...
        private static MimeMessage CreateMessage(string sender, string receivers, string subject, string body
            , bool isHTML = false, string ccs = null, string bccs = null, string attachments = null)
        {
            ...
            if (attachments?.Length > 0)
            {
                ...
                var files = attachments.Split(';').Select(path => new FileInfo(path.Trim())).ToList();
        ...
        public static string SendMailWithFiles(string attachments
        ...
        private static IList<UniqueId> SearchInbox(IMailFolder inbox, string beginTime, string endTime, string sender, string subject)
        {
            ...
            {
                var bdt = DateTime.ParseExact(beginTime, HyperMail.DATE_TIME_FORMAT, null);
                query = query.And(SearchQuery.DeliveredAfter(bdt));
            }

            if (endTime?.Trim().Length > 0)
            {
                var edt = DateTime.ParseExact(endTime, HyperMail.DATE_TIME_FORMAT, null);
        ...
        {
            var MailMessageDTO = new MailMessageDTO();
            var format = HyperMail.DATE_TIME_FORMAT;
